...
#[cfg(test)]
mod tests {
    //! Unit tests for the repovecd startup contract validation.
    ...
    fn validate_systemd_unit_contract_returns_err_on_invalid_units() {
        // Supply a minimal but deliberately broken repovecd unit (wrong ExecStart)
...
";
        let result = validate_systemd_units(target, broken_repovecd, mcpd);
        assert!(
