# Documentation contents

- [Documentation contents](contents.md): index of the repository documentation
  set.
- `docs/users-guide.md`: reserved for operator-facing and user-facing
  workflows when that guide is added.
- [Developers guide](developers-guide.md): maintainer workflow, quality gates,
  and repository automation policy.
- [repovec-appliance technical design](repovec-appliance-technical-design.md):
  architecture, constraints, and rationale for the appliance.
- [Roadmap](roadmap.md): staged implementation plan and task tracking.
- [Execution plans](execplans/1-1-3-ci-gating-pipeline.md): detailed delivery
  planning for larger roadmap tasks.
- [Complexity antipatterns and refactoring strategies](complexity-antipatterns-and-refactoring-strategies.md):
  design and refactoring reference for maintainers.
- [Documentation style guide](documentation-style-guide.md): repository
  standards for written documentation.
- [Ortho-config users guide](ortho-config-users-guide.md): reference material
  for the `ortho-config` dependency used by the project.
- [Reliable testing in Rust via dependency injection](reliable-testing-in-rust-via-dependency-injection.md):
  guidance on deterministic test design.
- [rstest-bdd users guide](rstest-bdd-users-guide.md): behavioural testing
  reference used by this repository.
- [Rust doctest DRY guide](rust-doctest-dry-guide.md): guidance for keeping
  Rust documentation examples maintainable.
- [Rust testing with rstest fixtures](rust-testing-with-rstest-fixtures.md):
  unit and integration testing reference.
- [Scripting standards](scripting-standards.md): repository guidance for
  scripts and CI glue.
